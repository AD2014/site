(function($){
	...
	function validateEmail(email){
		...
		if(strEmail == "") {
			...
			if ((strEmail.search(validRegExp) == -1)&&(strEmail != "")) 
			{
			   mailOk = false;
			} 
		}
    ...
    submitContact.on('click', function(e){
		...
		var messaggio = $('textarea#comment').val();

		var error = false;

		//check mandatory fields
		$("input.required").css("border-color","#e3e3e3");
		$("input.required").each(function() {
			
			if($(this).val() == "") {
				error = true;
				
				$(this).css("border-color", "#e96e66");
			}
		...
		if(error == false) {
			var bodyMsg = "Nuova richiesta di contatto da let.life/business<br>";
			bodyMsg += "nome: "+nome+"<br>azienda: "+azienda+"<br>email: "+email;
			...
			$.ajax({
				...
				async: false,
				headers: {
					"Authorization": "Basic dGVzdC1vYXV0aC1jbGllbnQ6dGVzdC1vYXV0aC1jbGllbnQ=" },
				cache: false,
				data: "message="+bodyMsg+"&subject="+subject,
				...
				error: function(data) {
					message.hide().removeClass('success').removeClass('error').addClass('error').html(data.responseText).fadeIn('slow').delay(5000).fadeOut('slow');
				}
			});
		}
    });
